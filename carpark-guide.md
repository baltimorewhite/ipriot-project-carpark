...
|---------------|--------------------------------------------------------|-----------------------------------------------------------------------|
| `CarPark`     | location, capacity, plates, displays sensors, log_file | add_car(plate_number), remove_car(plate_number), available_bays       |
| `Sensor`      | sensor_identifier, is_active, car_park                 | _scan_plate_number(), detect_vehicle(), update_car_park(plate_number) |
| `Display`     | display_identifier, message_text, is_display_on        | update(data_dictionary), __str__()                                    |
...
** It happens because I didn't use exacly the same names on my code

   ![Unit tests](screenshots/unit-tests-ready.png)
...
1. Add a screenshot of the output of the unit tests. If any failed, add a screenshot of the error message and a screenshot after you have fixed the errors:
   ...
   ![Unit tests](screenshots/unit-tests-display-ready.png)
...
> I would test it in the CarPark unit tests because the register() method is defined there, and it’s that class that validates the type of the object. Display and Sensor don't decide how they are registered — they are just used. long story short you test the logic where it lives — and that’s in CarPark.
